...
(defunc int/ (x y)
  :input-contract (and (integerp x)(integerp y)(not (equal 0 y)))
  :output-contract (integerp (int/ x y))
  (let* ((x-neg (< 0 x))
        (y-neg (< 0 y))
        (neg (and (or x-neg y-neg)(not (and x-neg y-neg)))) ;xor
        (posdiv (nat/ (abs x) (abs y))))
        (if neg
          (unary-- posdiv)
          posdiv
        )))
...
(defunc round (r)
  ...
  (let* ((absnum (abs (numerator r)))
         (absden (abs (denominator r)))
         (posfloor (nat/ absnum absden))
